	...
	History: 
		...
		- C++ was originally invented in 1979 and first version released in 1983
		...
		- After some years, the higher level languages like Java, Python, C# are used as a femous development
		  languages but as IOT and machince interfacing grows the execution speed of language become most important 
	...
	Clasic C++: (C++ before C++98): Additional data types, different IO style, function 			overloading, class and object,constructor and destructors, inheritance, polymorphism, operator 		overloading,file IO, exception handling,templets, STL...
...
int main()
{
	...
	std::cout<<"Address: F12, Divyadarshan society, behind kothrud bus stand, kothrud, Pune";
	return 0;
}
...
int main() {
	...
	Manipulators in C++: 
		In C++, manipulators are special functions or objects provided by the Standard Library that are used to modify the behaviour of input and output 
		streams. They are typically used in conjunction with the insertion (<<) and extraction (>>) operators to format input and output data. 
...
In above both cases, using const and constexpr, we declared constants, and when we tried to modify it, it will genberate an error 
message.
...
Unscoped Enum: there must be single constant name is same or different enums.
...
- Using #define Preprocessor Directive (C++98):
     ...
     -	When the source code is compiled, the preprocessor runs before the actual compilation starts. 
	The preprocessor scans the code for directives and performs the corresponding operations
     -	it help to manage code complexity,  

	For example: 	#define PI 3.14159	and 	#define MAX_SIZE 100


	Here are some of the most commonly used preprocessor directives:

	1)#include

	This directive is used to include the contents of another file into the current file. 
	It’s commonly used to include header files which contain declarations 
...
#include "myheader.h" // Includes a user-defined header file
	...
	This directive is used to define macros, which are constants or expressions that are replaced by their values throughout 
	the code.
Example:
...
#define SQUARE(x) ((x) * (x))
	...
	This directive is used to undefine a macro, effectively removing its definition.
Example:
...
#undef PI
	...
	These directives are used for conditional compilation. They allow the compiler to include or exclude parts of the code based 
...
int main() {
	...
	This directive can be used to generate a compilation error with a custom message. It’s useful for catching erroneous 
	configurations or conditions during the precompilation phase.

Example:


#ifdef WRONG_CONFIG
#error "Wrong configuration detected!"
#endif

	6)#pragma

	This directive is used to issue special commands to the compiler. The behavior of #pragma directives is compiler-specific.
Example:
...
#pragma once  // Ensures the file is included only once
	...
	*How Preprocessor Directives Work
	...
	File Inclusion: It replaces #include directives with the contents of the specified files.

	Macro Expansion: It replaces macros defined by #define with their corresponding values or expressions.

	Conditional Compilation: It evaluates conditional directives (#if, #ifdef, etc.) and includes or excludes parts of the code 
				accordingly.

	Error Handling: It checks for #error directives and stops the compilation if one is encountered, displaying the specified 
			error message.

	Special Commands: It processes #pragma directives according to compiler-specific rules.
...
In this example:
	...
	The #define PI 3.14159 directive defines a macro for the value of PI.
	The #ifdef DEBUG directive checks if the DEBUG macro is defined and includes the subsequent code block if it is.
	Preprocessor directives are a powerful tool in C and C++ that help manage code complexity, enhance portability, and 
	enable conditional compilation among other functionalities.
...
Input and Output in C++ using cin, cout: 
		   ...
		   Input	|	|			|	 	 Output
console input	    [ ]		|   |-->|	  ALU		|---|     |--->   [ ]
stream		     |		|   |	|_______________________|   |	  | console output stream
   c- scanf()	     |		|   |---|			|<--|	  | c - printf()
  c++ - cin	     |		|-------|	  IM		|	  | c++ - cout
		     |---------------->	|_______________________|---------|
				   	       ^          |	
	...
	cin (Standard Input): cin is a predefined object of the istream class, which represents the standard input stream. 
	It is used to read data from the standard input device, typically the keyboard, and store it into variables. 
	>> operator is actually bitwise right shift operator, which is overloaded here along with cin, to extract the 
	data from stream and therefore it is known as get-in or extraction operator.
...
cout (Standard Output): 
	cout is a predefined object of the ostream class, which represents the standard output stream. 
	It is used to output data to the standard output device, typically the console/screen. 
	<< operator is actually bitwise left-shift operator, which when used along with cout, 
	it will inserts / prints the data on screen and therefore it is called as insertion operator or put-to operator.
...
- Dynamic initilization: The  initialization refers to assigining
        the value to variable at the time of memory allocation. if the
        value to which variable is initialized is decided at runtime.
...
//----------------------------------------------------------------------------------------------------
	...
	The Boolean data type (bool keyword) used to hold the result of condition, it is value 1 when the condition is Ture and 
	0 when the condition is false. 
	C++20 introduces boolean literals true and false, which can be used to represent true and false values respectively.
...
//------------------------------------------------------------------------------------------------------------------
	...
	- we know that, the void data type is used to mention the function returning nothing in response to call.
	  Also when function does not collect any argument, then we use the void 
...
//---------------------------------------------------------------------------------------------------------------------------
 ...
 - Array: collection of elements having same data type, which conti. arranged
	...
	- character array: only one difference and it is
 ...
 - Pointer: It is used in the dynamic memory allocation to store an address
	...
	- Raw pointer: (Pointer we have seen in the C)
		The basic difference is instead of malloc() and free(),
...
In C++, a reference variable is an alias for another variable. It allows you to create a new name for an 
existing variable, which can be used to access or modify the value of the original variable. References provide
a way to work with variables indirectly, and they are particularly useful for passing arguments to functions efficiently.

Key Features of References

	Alias for Another Variable: 
		Once a reference is initialized to a variable, it becomes another name for 
		that variable. Any operations performed on the reference affect the original variable.

	Initialization: 
	 	References must be initialized when they are declared. Unlike pointers, they cannot be null 
		and cannot be reassigned to refer to a different variable once initialized.

	Syntax: The syntax for declaring a reference involves using the & symbol. 
	For example, int &ref = original; declares ref as a reference to the variable original.

	Access and Modification: References allow direct access to the original variable. 
	Modifications made through the reference will affect the original variable.

		int x=10;	
		int& y=x; // Here y is alternative name of x, called alise
		int z=x;
...
int add(int x, int y)
{
 ...
 //-------------------------------------------------------------------------------------------------
	...
	the function call is replaced with body of function. means it will avoid the push, pop and 
	...
	- virtual function can not be decl. as inline.
...
//=========================================================================================================
	...
	C++ allows you to define the member functions outside of class. in such case they are
...
//--------------------------------------------------------------------------------------------------------------------------------
	// Using static function with in class.
...
///----------------------------------------------------------------------------------
	...
	// Local Classes: These classes are decl. within the scope of any function, menas
			  as they are local to that function, these classes are termes as local
...
of functions. we we will see how to restrict transfer of multiple copies of same function.
...
int main()
{
 ...
 it is function from base class having empty body and qualified with virtual keyword.

When the function is made virtual, C++ determines which function is to be invoked at the runtime based on the type of the object 
...
Pure Virtual Function
	...
	- The "do-nothing" function is known as a pure virtual function. A pure virtual function is a function declared in the base class that has 
...
Limitations of Virtual Functions
	Slower: 
		The function call takes slightly longer due to the virtual mechanism and makes it more difficult for the compiler to optimize 
		because it does not know exactly which function is going to be called at compile time.

	Difficult to Debug: 
		In a complex system, virtual functions can make it a little more difficult to figure out where a function is being 
		called from.
...
//-------------------------------------------------------------
	...
	- Console Input Stream : This stream is created using the scanf() function in C, which is builtin
...
The ios class also contains few format flags used to format the output. It has format flags like showpos, showbase, oct, hex, etc. 
...
These built-in functions are available in the header file iomanip.h and ios class of header file iostream.h.
...
C++ provides a set of predefined manipulators. The header file iomanip.h header file contains these manipulators. Moreover, you can design your own 
manipulators in C++ to suit specific purpose. The user defined manipulators are defined as follows:
...
In C++, file I/O (Input/Output) involves reading from and writing to files. A "file" in this context typically refers to a named collection of data
stored on a storage medium, such as a hard drive or SSD.
...
To perform file I/O in C++, you typically follow these steps:
	...
	- Open the file: Use an instance of ifstream, ofstream, or fstream to open the file you want to read from or write to. 
	You specify the filename and optionally the file mode (e.g., ios::in for input, ios::out for output, ios::binary for binary mode, etc.).

	- Perform read or write operations: Once the file is open, you can use standard input/output operations (<< and >> operators) or member 
	  functions (getline(), write(), read(), etc.) to perform read or write operations on the file.

	- Close the file: After you're done with the file, it's essential to close it using the close() method of the file stream object. 
  	  This ensures that any buffers are flushed, and system resources associated with the file are released.
